...
(
    ...
    unique key uk_notebook_active_sibling (user_id, parent_key, node_type, name, active_key)
) engine = InnoDB
  default charset = utf8mb4;
...
alter table notebook add column if not exists parent_id bigint null after user_id;
alter table notebook add column if not exists parent_key bigint generated always as (coalesce(parent_id, 0)) stored after parent_id;
alter table notebook add column if not exists node_type varchar(16) not null default 'collection' after parent_id;
alter table notebook add column if not exists sort_order int not null default 0 after description;
alter table notebook add column if not exists active_key bigint generated always as (case when deleted = 0 then 0 else id end) stored after deleted;
update notebook set node_type = 'collection' where node_type is null or node_type = '';
create unique index uk_notebook_active_sibling on notebook (user_id, parent_key, node_type, name, active_key);
...
alter table paper_attachment add column if not exists extracted_text longtext null after file_size;
